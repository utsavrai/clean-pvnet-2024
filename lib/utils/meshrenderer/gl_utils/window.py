...
class Window(object):
    ...
    def __init__(self, window_width, window_height, samples=1, window_title='', monitor=1, show_at_center=True, offscreen=False):
        ...
        mon = glfw.get_monitors()[monitor] if monitor is not None else None
        self.windowID = glfw.create_window(window_width, window_height, self.window_title, mon, None)
        ...
        self.framebuf_width, self.framebuf_height = glfw.get_framebuffer_size(self.windowID)
        self.framebuffer_size_callback = []
        ...
        def framebuffer_size_callback(window, w, h):
            ...
            for callback in self.framebuffer_size_callback:
                callback(w, h)
        glfw.set_framebuffer_size_callback(self.windowID, framebuffer_size_callback)
        ...
        def key_callback(window, key, scancode, action, mode):
            ...
            for callback in self.key_callback:
                callback(key, scancode, action, mode)
        glfw.set_key_callback(self.windowID, key_callback)
        ...
        def mouse_callback(window, xpos, ypos):
            for callback in self.mouse_callback:
                callback(xpos, ypos)
        glfw.set_cursor_pos_callback(self.windowID, mouse_callback)
        ...
        def mouse_button_callback(window, button, action, mods):
            for callback in self.mouse_button_callback:
                callback(button, action, mods)
        glfw.set_mouse_button_callback(self.windowID, mouse_button_callback)
        ...
        def scroll_callback(window, xoffset, yoffset):
            for callback in self.scroll_callback:
                callback(xoffset, yoffset)
        glfw.set_scroll_callback(self.windowID, scroll_callback)
        ...
        if show_at_center:
            ...
            assert monitor >= 0 and monitor < len(monitors), 'Invalid monitor selected.'
            vidMode = glfw.get_video_mode(monitors[monitor])
            glfw.set_window_pos(self.windowID, 
                                vidMode.width // 2 - self.framebuf_width // 2, 
                                vidMode.height // 2 - self.framebuf_height // 2)
    ...
    def update_fps_counter(self):
        ...
        if elapsed_seconds > 1.0:
            self.previous_second = current_second
            fps = float(self.frame_count) / float(elapsed_seconds)
            glfw.set_window_title(self.windowID, '%s @ FPS: %.2f' % (self.window_title, fps))
            self.frame_count = 0.0
        self.frame_count += 1.0

    def is_open(self):
        return not glfw.window_should_close(self.windowID)
